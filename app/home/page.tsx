...
export default function InlyneHomepage() {
  ...
  // STEP 2: Fetch docs only if token is verified
  useEffect(() => {
    ...
    if (tokenValid !== true) {
      // Haven’t verified yet, or token is invalid (in which case we already redirected)
      return;
    }
  ...
  }, [token, maintenanceBypass, router, tokenValid]);

  // STEP 3: Fetch user mini-data only if token is verified
  useEffect(() => {
  ...
  // Show loading spinner while verifying or fetching
  if (loading || tokenValid !== true) {
            ...
            {filteredDocs.map(doc => (
              ...
              >
                ...
                <div className="p-4 text-center">
                  <h3 className="text-lg truncate inline">{doc.docTitle}</h3>
