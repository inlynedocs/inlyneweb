...
export default function DocHeader({
  ...
  return (
    <header className="flex items-center px-6 py-3 bg-white border-b shadow space-x-4">
      ...
      >
        {copied ? (
          <><Image src="/checkmark.svg" alt="✓" width={16} height={16} className="mr-1" />Copied</>
        ) : hoverCopy ? (
          <><Image src="/copy.svg" alt="✎" width={16} height={16} className="mr-1" />Copy</>
        ) : (
          <><span className="truncate">{docKey}</span><Image src="/link.svg" alt="🔗" width={16} height={16} className="ml-1" /></>
        )}
      </button>
